...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $yesterdaySales = Order::whereDate('created_at', $yesterday)->where('status', '!=', 'cancelled')->sum('total');
        $todayOrders   = Order::whereDate('created_at', $today)->where('status', '!=', 'cancelled')->count();
        $todayPaid     = Order::whereDate('created_at', $today)->where('status', '!=', 'cancelled')->sum('paid_amount');

        // ── Weekly & Monthly ──
        $weeklySales  = Order::where('created_at', '>=', $weekStart)->where('status', '!=', 'cancelled')->sum('total');
        $monthlySales = Order::where('created_at', '>=', $monthStart)->where('status', '!=', 'cancelled')->sum('total');
        $monthlyOrders = Order::where('created_at', '>=', $monthStart)->where('status', '!=', 'cancelled')->count();

        // ── Sales trend: change vs yesterday ──
        ...
        // ── Financial ──
        $totalReceivables = Customer::where('current_balance', '>', 0)->sum('current_balance');
        $totalAdvances    = Customer::where('current_balance', '<', 0)->sum(DB::raw('ABS(current_balance)'));
        ...
        $monthlyCost = OrderItem::whereHas('order', function ($q) use ($monthStart) {
        ...
        // ── Employees ──
        $presentEmployees = Attendance::whereDate('date', $today)->where('status', 'present')->count();
        ...
        // ── Top 5 customers by balance (owe the most) ──
        ...
        // ── Attendance ──
        $employeeAttendance = Attendance::with('employee.user')
