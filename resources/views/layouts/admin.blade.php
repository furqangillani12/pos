            ...
            <nav class="p-4 space-y-1 text-sm" x-data="{
                ...
                <div x-show="storefrontOpen || {{ $websiteOpen ? 'true' : 'false' }}" x-cloak class="ml-4 space-y-1">
                    ...
                    <a href="{{ route('admin.account-requests.index') }}"
                       class="block px-4 py-2 rounded-md transition {{ request()->routeIs('admin.account-requests.*') ? 'bg-cyan-100 text-cyan-700 font-semibold' : 'hover:bg-cyan-50 hover:text-cyan-700' }}">
                        <i class="fas fa-hand-holding-dollar mr-2 text-xs"></i> Pay / Withdraw
                        @php
                            try { $newAcctReq = \App\Models\AccountRequest::where('status', 'new')->count(); }
                            catch (\Throwable $e) { $newAcctReq = 0; }
                        @endphp
                        @if ($newAcctReq > 0)
                            <span class="inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 ml-1 rounded-full bg-amber-400 text-[10px] font-bold text-gray-900">{{ $newAcctReq }}</span>
                        @endif
                    </a>
